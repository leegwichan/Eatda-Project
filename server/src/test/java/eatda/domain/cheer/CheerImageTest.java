...
class CheerImageTest {

    @Nested
    class Validate {

        @Test
        void 응원_이미지가_정상적으로_생성된다() {
            CheerImage image = new CheerImage(null, "cheer/1.png", 0, "image/png", 12345L);

            assertThat(image.getImageKey()).isEqualTo("cheer/1.png");
        ...
        void 이미지키가_비어있으면_예외가_발생한다(String imageKey) {
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> new CheerImage(null, imageKey, 0, "image/png", 12345L));
        ...
        void 파일사이즈가_null이면_예외가_발생한다() {
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> new CheerImage(null, "cheer/1.png", 0, "image/png", null));
        ...
        void 파일사이즈가_0이하면_예외가_발생한다() {
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> new CheerImage(null, "cheer/1.png", 0, "image/png", 0L));
        ...
        void contentType이_null이면_예외가_발생한다() {
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> new CheerImage(null, "cheer/1.png", 0, null, 12345L));
